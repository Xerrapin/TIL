<template>
  <div class="header">
   ...
   store.js에 있는 state변경은 위에 처럼 하면 안된다. -->
    ...
    <button @click="$store.state.name" > 이름 변경</button>

    <!-- commit() : mutations 에게 요청 -->
    <button @click="$store.commit('ageinc', 5)" > 나이증가</button>
  ...
  Post 요청: 서버로 데이터를 보낼때 -->
   ...
   <button @click="$store.dispatch('getMore')">store더보기</button>
    <!-- dispatch() : actions 에게 요청 -->

    <!-- methods vs computed test -->
    <p> {{ nowMet() }} {{count}} </p>
    <p> {{ nowCom }} {{count}} </p> <!-- computed 함수는 이름만 쓴다. 초기값만 가져옴(실시간 변경x)-->
    <button @click="count++">날짜는 어떻니?</button>

    <p>{{ name }} </p>
    <p>{{ gender }} </p>
    <p>{{ age }} </p>
    <p>{{ likes }} </p>
    <button @click="$store.namechage">이름 변경</button>
...
export default {
  ...
  // 함수 만들때 :{}, computed:{}
  computed: { // 처음 실행하고 값을 간직함. 계산 결과 저장용 함수들~
    nowCom(){
      return new Date()
    },
    // name(){
    ...
    // },
    ...mapState(['name', 'age', 'likes', 'gender', 'ageinc']), // 배열타입으로 가져오기
    ...mapState({name:'kingsmile'}),
    ...mapState({ageinc:30})
    // ...mapState({namechange:'kingsmile'}),
    // ...mapState({namming:'name', myage:'age'}) // 객체타입
