...
--    단, 학과명순으로 오름차순 정렬. 
SELECT profno "교수번호", name "이름", hiredate "입사일", d.dname 학과명
...
CREATE TABLE c_professor
    AS SELECT name "이름", position "직위", hiredate "입사일", email "이메일", hpage "홈페이지"
    FROM professor
    WHERE 1=0;
...
--문제8] c_emp22(emp?) 테이블의 구조물만 복사 c_emp22로 만드세요
SELECT * FROM emp;
